    ...
    <div className="flex flex-col h-full gap-6 animate-fade-in pb-8">
      ...
      {showDeptModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-2xl transition-colors duration-300">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">Gestionar Departamentos</h3>
              <div className="flex items-center gap-2">
                <button onClick={() => { setShowDeptModal(false); setEditingDept(false); setDeptForm({ name: '', encargado: '' }); }} className="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">Cerrar</button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input name="name" value={deptForm.name} onChange={handleDeptFormChange} className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white" />
              </div>
              <div>
                <label className="block text-sm font-semibold mb-1 text-gray-700 dark:text-gray-300">Encargado</label>
                <input name="encargado" value={deptForm.encargado} onChange={handleDeptFormChange} className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white" />
              </div>
            </div>
            <div className="flex gap-2 mb-4">
              ...
              <button onClick={() => { setDeptForm({ name: '', encargado: '' }); setEditingDept(false); }} className="px-4 py-2 rounded border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">Limpiar</button>
            </div>

            <div>
              <h4 className="font-semibold mb-2 text-gray-900 dark:text-white">Departamentos existentes</h4>
              <div className="space-y-2 max-h-64 overflow-auto">
                {departmentsList.length === 0 ? (
                  <p className="text-sm text-gray-500 dark:text-gray-400">No hay departamentos registrados.</p>
                ) : (
                  departmentsList.map(d => (
                    <div key={d.name} className="flex justify-between items-center p-2 border border-gray-300 dark:border-gray-600 rounded bg-gray-50 dark:bg-gray-700/50">
                      <div>
                        <div className="font-semibold text-gray-900 dark:text-white">{d.name}</div>
                        <div className="text-sm text-gray-600 dark:text-gray-400">Encargado: {d.encargado}</div>
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => handleDeptEdit(d)} className="text-sm px-3 py-1 rounded border border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300">Editar</button>
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-white dark:bg-gray-800 p-2 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
        ...
        <div className="flex bg-gray-100 dark:bg-gray-700 p-1 rounded-xl">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-lg font-bold text-sm transition-all ${activeTab === 'fallas'
          ...
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-lg font-bold text-sm transition-all ${activeTab === 'bienes'
                ? 'bg-white dark:bg-[#172554] text-blue-600 dark:text-white shadow-md'
                : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'
              }`}
          >
                          ...
                          <td className="p-5">
                            {userIsAdmin ? (
                              <button
                                ...
                                className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold cursor-pointer transition-all hover:scale-105 ${item.estado === 'Resuelto'
                              ...
                              <span className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold ${item.estado === 'Resuelto'
                                  ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'
                                  : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400'
                                }`}>
                                {item.estado === 'Resuelto' ? <FaCheckCircle /> : <FaClock />}
